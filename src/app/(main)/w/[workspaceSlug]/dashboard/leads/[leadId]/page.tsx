...
export default async function LeadPage({ params, searchParams }: LeadPageProps) {
  const authorization = await getPermittedDashboardAuthorization("leads.view");
  ...
  const canUpdateLead = can(membership, "leads.update");
  const canDeleteLead = can(membership, "leads.delete");
  const canSendEmail = can(membership, "email.send");
  const canManageGmailAccount = can(membership, "email.account.manage");
  const canCreateEstimate = can(membership, "estimates.create");
  const canViewEstimates = can(membership, "estimates.view");
  const canConvertLead = canUpdateLead && can(membership, "customers.create");

  const { leadId } = await params;
  const resolvedSearchParams = await searchParams;
  const [lead, workspace, googleMailAccount] = await Promise.all([
  ...
  const templates = canSendEmail
    ? await getRenderedDocumentEmailTemplates({
        ...
        scope: "lead",
        context: {
          companyEmail: workspace.companyEmail ?? workspace.legacyOwner?.email,
  ...
  const notice = !canSendEmail
    ? null
    : resolvedSearchParams?.gmail_connected
    ...
    <div className="@container/main mx-auto grid w-full max-w-6xl gap-4 md:gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {canUpdateLead ? (
          ...
          <div className="flex flex-wrap items-center gap-2">
            <PermissionDisabledButton size="sm" reason="Your role cannot change lead status.">
              <CircleCheckBig />
              Mark won
            </PermissionDisabledButton>
            <PermissionDisabledButton size="sm" variant="destructive" reason="Your role cannot change lead status.">
              <XCircle />
              Mark lost
            </PermissionDisabledButton>
          </div>
        ...
        <CardHeader className="gap-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center [&_button]:w-full sm:[&_button]:w-auto">
            {canUpdateLead ? (
              ...
              <PermissionDisabledButton size="sm" reason="Your role can view leads but cannot edit them.">
                <Pencil />
                Edit lead
              </PermissionDisabledButton>
            )}
            {canSendEmail ? (
              ...
              <PermissionDisabledButton size="sm" reason="Your role cannot create estimates.">
                <ReceiptText />
                Create estimate
              </PermissionDisabledButton>
            ) : null}
            {lead.estimateRecordId && canViewEstimates ? (
            ...
            ) : !lead.estimateRecordId && canCreateEstimate ? (
              <Button asChild size="sm" className="w-full sm:w-auto">
            ...
            {canConvertLead ? <ConvertLeadButton action={convertLeadToCustomerAction} lead={lead} /> : null}
          </div>
        </CardHeader>
        <CardContent className="grid gap-4">
          {canSendEmail && !lead.email ? <LeadEmailWarning /> : null}
