...
export default async function Page() {
  ...
  const leads = await getLeads(authorization.workspaceId);
  const summary = summarizeLeads(leads);
  const canCreateLeads = can(authorization.membership, "leads.create");
        ...
        <CardHeader className="gap-2 pb-3 sm:gap-4 sm:pb-6">
          ...
          <CardAction>
            <CreateLeadDialog
              ...
              disabledReason={canCreateLeads ? undefined : "Your role can view leads but cannot create them."}
            />
          </CardAction>
        </CardHeader>
