...
import { ArrowRight, Hammer, Package, Pencil, Search } from "lucide-react";
...
export function ServicesDashboard({ canManage, services }: { canManage: boolean; services: ServiceTemplateRow[] }) {
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {filteredServices.map((service) => {
            ...
            const card = (
              <Card
                key={service.id}
                className="h-full overflow-hidden rounded-lg transition-colors group-hover:bg-muted/20"
                size="sm"
              >
                <CardContent className="grid h-full min-w-0 grid-cols-[minmax(0,1fr)] gap-4 overflow-hidden p-4">
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    ...
                    {canManage ? (
                      ...
                      <PermissionDisabledButton
                        ...
                        size="icon"
                        className="size-8"
                        reason="Your role can view services but cannot edit them."
                        aria-label={`Edit ${service.title} unavailable`}
                      >
                        <Pencil />
                      </PermissionDisabledButton>
                    )}
                  ...
                  <div className="mt-auto grid min-w-0 grid-cols-3 gap-2 rounded-md bg-muted/20 p-3 text-sm">
                    <div className="grid min-w-0 gap-1">
                      ...
                      <span className="truncate font-medium tabular-nums">{formatMoney(laborSubtotal.toFixed(2))}</span>
                    </div>
                    <div className="grid min-w-0 gap-1">
