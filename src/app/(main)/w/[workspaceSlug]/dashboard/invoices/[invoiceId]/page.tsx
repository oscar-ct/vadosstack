...
export default async function Page({
  ...
  const [companyLogoSrc, successfulEmailCount, workspace] = await Promise.all([
  ...
  const canDeleteInvoice = can(authorization.membership, "invoices.delete");
  const canEditInvoice = can(authorization.membership, "invoices.update");
  const canEditJob = can(authorization.membership, "jobs.update");
  ...
  const canRecordPayment = can(authorization.membership, "invoices.record_payment");
  const canSendInvoice = can(authorization.membership, "invoices.send");
  ...
  const companyAddress = workspace.companyAddress?.trim();
  const companyEmail = workspace.companyEmail ?? workspace.legacyOwner?.email ?? authorization.principal.user.email;
  ...
  const invoiceMessage = workspace.invoiceMessageEnabled
    ? renderDocumentMessage(workspace.invoiceMessageText, invoiceMessageContext)
    : "";
  const invoiceMessageLines = getDocumentMessageLineItems(invoiceMessage);
  const invoiceMessageAlign = normalizeDocumentMessageAlign(workspace.invoiceMessageAlign);
  const googleMailAccount = canSendInvoice
    ? await prisma.googleMailAccount.findUnique({ where: { workspaceId } })
    : null;
  ...
  const gmailNotice = resolvedSearchParams?.gmail_connected
    ...
    : gmailError
      ...
      : null;
  const emailTemplates = canSendInvoice
    ? await getRenderedDocumentEmailTemplates({
        ...
        context: {
          ...
          companyEmail,
          companyName: workspace.name,
          companyPhone: workspace.companyPhone ? formatPhoneNumber(workspace.companyPhone) : undefined,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <InvoiceActions
            ...
            invoiceMessageEnabled={workspace.invoiceMessageEnabled}
            invoiceMessageText={workspace.invoiceMessageText}
            ...
            <div className="grid gap-1">
              <div className="mb-2 flex items-start gap-3">
                ...
                <div className="grid gap-0.5">
                  <div className="font-semibold text-lg leading-none print:text-sm">{workspace.name}</div>
                  {companyAddress ? (
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-muted-foreground text-xs">
                    ...
                    {workspace.companyPhone ? (
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        <span>{formatPhoneNumber(workspace.companyPhone)}</span>
                      </span>
                    ) : null}
                  </div>
                </div>
