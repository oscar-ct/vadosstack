...
export default async function CustomerPage({ params, searchParams }: CustomerPageProps) {
  ...
  const canViewEmailHistory = can(authorization.membership, "email.history.view");
  const canUpdateCustomer = can(authorization.membership, "customers.update");
  const canDeleteCustomer = can(authorization.membership, "customers.delete");
  ...
  const customer = await prisma.customer.findUnique({
    ...
    include: {
      addresses: true,
      estimateRecords: {
        where: canViewEstimates ? undefined : { id: { in: [] } },
        include: {
          printableEstimate: canViewEstimates,
        },
        orderBy: {
          createdAt: "desc",
        },
      ...
      jobs: {
        where: canViewJobs ? undefined : { id: { in: [] } },
        include: {
          invoice: canViewInvoices,
      ...
      orders: {
        where: canViewOrders ? undefined : { id: { in: [] } },
        include: {
          items: true,
          returns: {
            include: {
  ...
  const emailRecords =
    canViewEmailHistory && emailRecordFilters.length
