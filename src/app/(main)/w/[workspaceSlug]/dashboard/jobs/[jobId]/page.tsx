...
export default async function Page({
  ...
  const job = await getJob(authorization.workspaceId, jobId);
  ...
  const canViewPayments = can(authorization.membership, "invoices.view");
  const canManagePayments = can(authorization.membership, "invoices.record_payment");
  const deposits = canViewPayments ? (job?.payments ?? []).filter((payment) => payment.paymentType === "deposit") : [];
    ...
    <div className="@container/main mx-auto grid w-full max-w-7xl gap-5 md:gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {can(authorization.membership, "jobs.complete") ? (
            <MarkJobCompleteButton
          ...
          {can(authorization.membership, "jobs.update") ? (
            ...
            <PermissionDisabledButton
              size="sm"
              variant="outline"
              reason="Your role can view jobs but cannot edit them."
            >
              <Pencil />
              Edit
            </PermissionDisabledButton>
          )}
          {can(authorization.membership, "invoices.record_payment") ? (
            <CreateDepositButton action={createJobPaymentAction} className="w-auto" size="sm" job={job} />
          ) : null}
          {can(authorization.membership, "invoices.create") ? (
            <JobInvoiceButton action={createInvoiceAction} className="w-auto" job={job} size="sm" />
          ) : !job.invoiceId ? (
            <PermissionDisabledButton size="sm" reason="Your role cannot create invoices.">
              <ReceiptText />
              Create invoice
            </PermissionDisabledButton>
          ) : null}
