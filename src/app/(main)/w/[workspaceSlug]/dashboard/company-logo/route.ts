...
export async function GET() {
  ...
  if (logo?.companyLogoKey) {
    try {
      const publicUrl = getR2PublicUrl(logo.companyLogoKey);

      if (publicUrl) {
        return redirectResponse(publicUrl);
      }

      const object = await getR2Object(logo.companyLogoKey);
      const bytes = await object.Body?.transformToByteArray();

      if (bytes) {
        return imageResponse(
          Buffer.from(bytes),
          object.ContentType ?? logo.companyLogoType ?? "application/octet-stream",
        );
      }
    } catch {
      return fallbackResponse();
  ...
  const logoDataUrl = logo?.companyLogoDataUrl;
