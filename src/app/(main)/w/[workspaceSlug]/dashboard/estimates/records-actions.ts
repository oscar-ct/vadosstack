...
class EstimateActionError extends Error {}

function getEstimateActionErrorMessage(error: unknown, fallback: string) {
  if (error instanceof EstimateActionError) {
...
async function assertCustomer(ownerId: string, customerId: string) {
  ...
  if (!customer) {
    throw new EstimateActionError("Select a customer from your account.");
  }
}
...
async function createCustomerForEstimate({
  ...
  if (!name || !phone) {
    throw new EstimateActionError("Enter a customer name and phone number before creating the estimate.");
  }

  const normalizedPhone = normalizePhoneNumber(phone);

  if (normalizedPhone.length !== 10) {
    throw new EstimateActionError("Enter a valid 10-digit phone number before creating the estimate.");
  }
  ...
  if (parsedEmail && !parsedEmail.success) {
    throw new EstimateActionError(parsedEmail.error.issues[0]?.message ?? "Enter a valid customer email.");
  }
...
async function createLeadForEstimate({
  ...
  if (!name) {
    throw new EstimateActionError("Enter a lead name before creating the estimate.");
  }

  const normalizedPhone = normalizePhoneNumber(phone);

  if (phone && normalizedPhone.length !== 10) {
    throw new EstimateActionError("Enter a valid 10-digit lead phone number.");
  }
...
async function upsertPrintableEstimateSnapshotFromRecord(
  ...
  if (!estimate) {
    throw new EstimateActionError("Estimate not found.");
  }
...
export async function createEstimateRecordAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to create estimates." };
  }
  ...
  if (parsed.data.newCustomerName && !can(authorization.membership, "customers.create")) {
    return { success: false, message: "You do not have permission to create customers." };
  }

  if (parsed.data.newLeadName && !can(authorization.membership, "leads.create")) {
  ...
  try {
    ...
    const createdEstimate = await prisma.$transaction(async (tx) => {
      ...
      if (estimate.status === "Ready to Send" || estimate.status === "Waiting on Customer") {
        await upsertPrintableEstimateSnapshotFromRecord(tx, createdEstimate.id, workspaceId);
      }
      ...
      await recordAuthorizationAuditEvent(
        {
          ...
          targetType: "EstimateRecord",
          targetId: createdEstimate.id,
        },
        tx,
      );
    ...
    return { success: false, message: getEstimateActionErrorMessage(error, "Estimate could not be created.") };
  }
  ...
  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/leads");
...
export async function updateEstimateRecordAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to update estimates." };
  }
  const workspaceId = authorization.workspaceId;
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (estimate.status === "Draft" && existingEstimate.printableEstimate) {
        ...
        if (!existingEstimate.printableEstimate || customerDocumentChanged) {
          syncedPrintableEstimateId = await upsertPrintableEstimateSnapshotFromRecord(tx, id, workspaceId);
    ...
    return { success: false, message: getEstimateActionErrorMessage(error, "Estimate could not be updated.") };
  }
  ...
  if (syncedPrintableEstimateId) {
    revalidateWorkspacePath(
      ...
      `/dashboard/estimates/${syncedPrintableEstimateId}`,
    );
  }
...
export async function deleteEstimateRecordAction(
  ...
  const authorization = await getPermittedDashboardAuthorization("estimates.delete");

  if (!authorization) {
    return { success: false, message: "You do not have permission to delete estimates." };
  }
  const workspaceId = authorization.workspaceId;
  ...
  try {
    ...
    return { success: false, message: getEstimateActionErrorMessage(error, "Estimate could not be deleted.") };
  }

  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/estimates");
  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/customers");
  redirect(scopeWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/estimates"));
}
...
export async function updateEstimateStatusAction(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (parsed.data.status === "Draft" && estimate.printableEstimate) {
        ...
        if (estimate.printableEstimate) {
          ...
          await upsertPrintableEstimateSnapshotFromRecord(tx, parsed.data.id, workspaceId);
        }
    ...
    return {
      success: false,
      message: getEstimateActionErrorMessage(error, "Estimate status could not be updated."),
    };
  }

  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/estimates");
  revalidateWorkspacePath(authorization.membership.workspaceSlug, `/dashboard/estimates/records/${parsed.data.id}`);
  return {
    success: true,
    message:
      parsed.data.status === "Waiting on Customer"
...
export async function convertEstimateToJobAction(
  ...
  const authorization = await getPermittedDashboardAuthorization("estimates.convert");

  if (!authorization) {
    return { success: false, message: "You do not have permission to convert estimates." };
  }
  const workspaceId = authorization.workspaceId;

  if (!can(authorization.membership, "jobs.create")) {
    return { success: false, message: "You do not have permission to create jobs." };
  }
  ...
  try {
    const conversion = await prisma.$transaction(async (tx) => {
      ...
      if (estimate.convertedJobId) {
        throw new EstimateActionError("This estimate has already been converted to a job.");
      }

      if (estimate.status !== "Waiting on Customer") {
        throw new EstimateActionError("Only estimates waiting on a customer decision can be converted to a job.");
      }

      let customerId = estimate.customerId;

      if (!customerId && estimate.lead) {
        if (!can(authorization.membership, "customers.create")) {
          throw new EstimateActionError("You do not have permission to create the customer required for this job.");
        }
      ...
      if (!customerId) {
        throw new EstimateActionError("Add a customer or lead before converting this estimate to a job.");
      }
      ...
      if (updatedEstimate.count !== 1) {
        throw new EstimateActionError("This estimate was already updated. Refresh the page and try again.");
      }
      ...
      await syncCustomerBillingStatus(customerId, workspaceId, tx);
      await recordAuthorizationAuditEvent(
        {
          workspaceId,
          actorUserId: authorization.principal.user.id,
    ...
    return { success: false, message: getEstimateActionErrorMessage(error, "Estimate could not be converted.") };
  }

  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/estimates");
  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/jobs");
  revalidateWorkspacePath(authorization.membership.workspaceSlug, `/dashboard/jobs/${convertedJobId}`);
  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/customers");
  return {
    success: true,
...
export async function createPrintableEstimateAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to publish estimates." };
  }
  const workspaceId = authorization.workspaceId;
  ...
  try {
    const printableEstimateId = await prisma.$transaction(async (tx) => {
      ...
      const printableEstimateId = await upsertPrintableEstimateSnapshotFromRecord(tx, id, workspaceId);
      await recordAuthorizationAuditEvent(
        {
          workspaceId,
          actorUserId: authorization.principal.user.id,
          membershipId: authorization.membership.id,
          action: "estimate.publish",
          targetType: "EstimateRecord",
          targetId: id,
          metadata: { printableEstimateId },
        },
        tx,
      );
      return printableEstimateId;
    });
    revalidateWorkspacePath(authorization.membership.workspaceSlug, `/dashboard/estimates/${printableEstimateId}`);
  } catch (error) {
    return {
      success: false,
      message: getEstimateActionErrorMessage(error, "Estimate could not be published."),
    };
  }

  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard/estimates");
  revalidateWorkspacePath(authorization.membership.workspaceSlug, `/dashboard/estimates/records/${id}`);
  redirect(
    scopeWorkspacePath(authorization.membership.workspaceSlug, `/dashboard/estimates/records/${id}?view=customer`),
  );
}
