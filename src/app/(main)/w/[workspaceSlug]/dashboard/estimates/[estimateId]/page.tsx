...
export default async function Page({
  ...
  if (estimate.estimateRecordId) {
    ...
    redirect(
      scopeWorkspacePath(
        ...
        `/dashboard/estimates/records/${estimate.estimateRecordId}?${canonicalParams.toString()}`,
      ),
    );
  }
  ...
  const estimateMessage = workspace.estimateMessageEnabled
    ? renderDocumentMessage(workspace.estimateMessageText, estimateMessageContext)
    : "";
  const estimateMessageLines = getDocumentMessageLineItems(estimateMessage);
  const estimateMessageAlign = normalizeDocumentMessageAlign(workspace.estimateMessageAlign);
  const googleMailAccount = canSendEstimate
  ...
  const gmailNotice = resolvedSearchParams?.gmail_connected
    ...
    : gmailError
      ...
      : null;
  const emailTemplates = canSendEstimate
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <EstimateActions
            ...
            customerEmail={estimate.customerEmail}
            customerName={estimate.customerName}
            deleteAction={canDeleteEstimate ? deleteEstimateAction : undefined}
            ...
            editHref={canEditEstimate ? editHref : undefined}
            editLabel={editLabel}
            estimateId={estimate.id}
            estimateMessageAlign={estimateMessageAlign}
            estimateMessageEnabled={workspace.estimateMessageEnabled}
            estimateMessageText={workspace.estimateMessageText}
