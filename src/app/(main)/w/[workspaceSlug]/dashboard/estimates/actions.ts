...
export async function emailEstimateAction(
  ...
  const authorization = await getPermittedDashboardAuthorization("estimates.send");

  if (!authorization) {
    return createEmailEstimateState(false, "You do not have permission to send estimates.");
  }
  ...
  const [estimate, googleMailAccount, workspace] = await Promise.all([
  ...
  try {
    ...
    const responseMessage = reconnectRequired
      ...
      : "Estimate email could not be sent. Please try again.";
  ...
  revalidateWorkspacePath(authorization.membership.workspaceSlug, `/dashboard/estimates/${estimate.id}`);
  if (estimate.estimateRecordId) {
    revalidateWorkspacePath(
      authorization.membership.workspaceSlug,
      `/dashboard/estimates/records/${estimate.estimateRecordId}`,
    );
  }
...
export async function deleteEstimateAction(
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (estimate.estimateRecord?.convertedJobId) {
        throw new EstimateActionError("A converted estimate cannot be unpublished while its job exists.");
      }
    ...
    return {
      success: false,
      message:
        error instanceof EstimateActionError ? error.message : "Estimate could not be deleted. Please try again.",
    };
  }
  ...
  if (unpublishedEstimateRecordId) {
    revalidateWorkspacePath(
      authorization.membership.workspaceSlug,
      `/dashboard/estimates/records/${unpublishedEstimateRecordId}`,
    );
  }
