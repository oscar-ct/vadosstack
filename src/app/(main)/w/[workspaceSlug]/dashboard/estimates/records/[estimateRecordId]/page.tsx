...
export default async function Page({
  ...
  const [recordWorkspace, companyWorkspace] = await Promise.all([
    getEstimateRecordWorkspace(workspaceId, estimateRecordId),
  ...
  if (!recordWorkspace || !companyWorkspace) {
    notFound();
  }

  const { estimate, source } = recordWorkspace;
  ...
  const validThroughDate = publishedEstimate
    ? addDays(publishedEstimate.issuedAt, companyWorkspace.estimateValidDays)
    : null;
  ...
  const estimateMessage = companyWorkspace.estimateMessageEnabled
    ? renderDocumentMessage(companyWorkspace.estimateMessageText, estimateMessageContext)
    : "";
  const estimateMessageLines = getDocumentMessageLineItems(estimateMessage);
  const estimateMessageAlign = normalizeDocumentMessageAlign(companyWorkspace.estimateMessageAlign);
  const companyEmail =
    companyWorkspace.companyEmail ?? companyWorkspace.legacyOwner?.email ?? authorization.principal.user.email;
  const canDeleteEstimate = can(authorization.membership, "estimates.delete");
  const canEditEstimate = can(authorization.membership, "estimates.update");
  const canManageGmailAccount = can(authorization.membership, "email.account.manage");
  const canManageDocumentMessage = can(authorization.membership, "settings.manage");
  const canSendEstimate = can(authorization.membership, "estimates.send");
  const canConvertEstimate =
    can(authorization.membership, "estimates.convert") && can(authorization.membership, "jobs.create");
  const [companyLogoSrc, googleMailAccount, emailTemplates] = await Promise.all([
    getCompanyLogoSrc(workspaceId),
    canSendEstimate
    ...
    canSendEstimate
      ? getRenderedDocumentEmailTemplates({
          ...
          context: {
            companyEmail,
            companyName: companyWorkspace.name,
            companyPhone: companyWorkspace.companyPhone ? formatPhoneNumber(companyWorkspace.companyPhone) : undefined,
            customerEmail,
            customerName,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {canEditEstimate ? (
            ...
            <PermissionDisabledButton size="sm" reason="Your role can view estimates but cannot edit them.">
              <Pencil />
              Edit
          ...
          {canEditEstimate && estimate.status === "Ready to Send" && publishedEstimate && !customerEmail ? (
            <UpdateEstimateStatusButton
          ...
          {canConvertEstimate && estimate.status === "Waiting on Customer" ? (
            <ConvertEstimateButton
          ...
          {canEditEstimate && estimate.status === "Waiting on Customer" ? (
            <UpdateEstimateStatusButton
              action={updateEstimateStatusAction}
              className="w-auto"
              estimate={estimate}
          ...
          {canConvertEstimate && estimate.status === "Won" && estimate.convertedJobId ? (
            <ConvertEstimateButton
              action={convertEstimateToJobAction}
              className="w-auto"
              estimate={estimate}
          ...
          {canEditEstimate && estimate.status === "Lost" ? (
            <UpdateEstimateStatusButton
              action={updateEstimateStatusAction}
              className="w-auto"
              estimate={estimate}
          ...
          <EstimateActions
            ...
            canManageDocumentMessage={canManageDocumentMessage}
            canSend={canSendEstimate}
            compact
            companyName={companyWorkspace.name}
            customerEmail={customerEmail}
            customerName={customerName}
            deleteAction={canDeleteEstimate ? deleteEstimateRecordAction : undefined}
            ...
            editHref={canEditEstimate ? `/dashboard/estimates/records/${estimate.id}/edit` : undefined}
            estimateId={publishedEstimate?.id}
            estimateMessageAlign={estimateMessageAlign}
            estimateMessageEnabled={companyWorkspace.estimateMessageEnabled}
            estimateMessageText={companyWorkspace.estimateMessageText}
      ...
      <EstimateRecordViews
        ...
        customerPreview={
          <EstimateCustomerPreview
            ...
            companyName={companyWorkspace.name}
            companyPhone={companyWorkspace.companyPhone}
            data={{
              customerEmail,
              customerName,
              customerPhone,
