  ...
  return (
    <div className="grid min-w-0 gap-5">
      ...
      <section className="grid gap-4 rounded-lg border bg-muted/15 p-4">
        ...
        <div className={cn("grid gap-4", canManageCompensation ? "md:grid-cols-4" : "md:grid-cols-2")}>
          <div className="grid min-w-0 gap-2">
        ...
        <DialogContent className="top-0 left-0 grid h-svh max-h-svh w-screen max-w-none translate-x-0 translate-y-0 grid-rows-[auto_minmax(0,1fr)] gap-0 overflow-hidden rounded-none p-0 sm:top-1/2 sm:left-1/2 sm:h-auto sm:max-h-[calc(100svh-2rem)] sm:w-[calc(100vw-2rem)] sm:max-w-3xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-xl">
          ...
          >
            <div className="grid min-h-0 gap-4 overflow-y-auto overflow-x-hidden p-4">
              <EmployeeFormFields canManageCompensation={canManageCompensation} employee={employee} />
      ...
      <Card className="overflow-hidden rounded-lg">
        <CardHeader className="border-b bg-muted/20">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            {canManage ? (
              <EmployeeDialog action={createAction} canManageCompensation={canManageCompensation} />
            ) : (
              <PermissionDisabledButton size="sm" reason="Your role can view employees but cannot add them.">
                <Plus />
                Add employee
                ...
                <Card key={employee.id} className="gap-0 rounded-lg" size="sm">
                  <CardContent className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 overflow-hidden p-4">
                    <div className="min-w-0">
                      ...
                      {canManageAccess && employee.dashboardAccess ? (
                        <div className="mt-2 flex flex-wrap items-center gap-1.5">
                          <Badge variant={employee.dashboardAccess.status === "Active" ? "secondary" : "outline"}>
                            {employee.dashboardAccess.status === "Active"
                              ? employee.dashboardAccess.roleName
                              : employee.dashboardAccess.status}
                          </Badge>
                          {employee.dashboardAccess.status !== "Active" && employee.dashboardAccess.roleName ? (
                            <Badge variant="outline">{employee.dashboardAccess.roleName}</Badge>
                          ) : null}
                          {employee.dashboardAccess.isCustomRole ? <Badge variant="outline">Custom</Badge> : null}
                        </div>
                      ) : null}
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 rounded-md border bg-muted/15 px-3 py-2">
                      {canManage ? (
                        ...
                        <Badge variant="outline">{employee.active ? "Active" : "Inactive"}</Badge>
                      )}
                      <div className="flex items-center gap-2">
                        {canManage ? (
                          <EmployeeColorPicker
