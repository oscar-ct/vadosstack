...
function revalidateEmployeePaths(workspaceSlug: string, employeeId?: string) {
  revalidateWorkspacePath(workspaceSlug, "/dashboard/employees");
  if (employeeId) revalidateWorkspacePath(workspaceSlug, `/dashboard/employees/${employeeId}`);
  revalidateWorkspacePath(workspaceSlug, "/dashboard/time-tracking");
}
...
export async function createEmployeeAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to add an employee." };
  }
  ...
  const employeeNumber = parsed.data.employeeNumber ?? (await generateEmployeeNumber(workspaceId));
  const accentColor = await getNextEmployeeAccent(workspaceId);

  if (await employeeNumberExists(workspaceId, employeeNumber)) {
  ...
  await prisma.employee.create({
    data: {
      ...
      payType: canManageCompensation ? parsed.data.payType : "Hourly",
      payRate: canManageCompensation ? (parsed.data.payRate ?? null) : null,
  ...
  revalidateEmployeePaths(authorization.membership.workspaceSlug);
...
export async function updateEmployeeAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to update an employee." };
  }
  const workspaceId = authorization.workspaceId;
  const canManageCompensation = can(authorization.membership, "employees.compensation.manage");
  ...
  if (await employeeNumberExists(workspaceId, parsed.data.employeeNumber, parsed.data.employeeId)) {
    return { success: false, message: "That employee number is already in use." };
  }
  ...
  revalidateEmployeePaths(authorization.membership.workspaceSlug, parsed.data.employeeId);
...
export async function updateEmployeeAccentAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to update an employee color." };
  }
...
export async function deleteEmployeeAction(
  ...
  if (!authorization) {
    return { success: false, message: "You do not have permission to delete an employee." };
  }
  const workspaceId = authorization.workspaceId;
