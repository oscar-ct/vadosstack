...
export async function sendGeneralEmailAction(
  ...
  if (!authorization) {
    return createGeneralEmailState(false, "You do not have permission to send email.");
  }
  ...
  }).catch((error) => console.error("Email authorization audit could not be saved.", error));
...
export async function updateCompanySettingsAction(
  ...
  const brandingChangeRequested = Boolean(parsed.data.deleteLogo) || (logoFile instanceof File && logoFile.size > 0);

  if (brandingChangeRequested && !(await getPermittedDashboardAuthorization("settings.branding.manage"))) {
    return {
      success: false,
      message: "You do not have permission to change company branding.",
    };
  }
  ...
  try {
    logoUpload = await getLogoUpload(logoFile, workspaceId);
  } catch (error) {
    return {
      success: false,
  ...
  try {
    ...
    workspaceSlug = await prisma.$transaction(async (transaction) => {
      const nextSlug = await updateWorkspaceNameAndSlug(transaction, workspaceId, parsed.data.companyName);
      ...
      await recordAuthorizationAuditEvent(
        {
          ...
          metadata: { brandingChanged: brandingChangeRequested },
        },
        transaction,
      );
      return nextSlug;
    ...
    if ((parsed.data.deleteLogo || logoUpload) && existingWorkspace?.companyLogoKey) {
      await deleteR2Object(existingWorkspace.companyLogoKey).catch(() => undefined);
    }
  } catch (error) {
    if (logoUpload) {
    ...
    return {
      success: false,
      message: "Company settings could not be updated. Please try again.",
    };
  }

  revalidateWorkspacePath(authorization.membership.workspaceSlug, "/dashboard");
...
export async function updateAccountProfileAction(
  ...
  for (const membership of principal.memberships) {
    revalidateWorkspacePath(membership.workspaceSlug, "/dashboard");
  }
