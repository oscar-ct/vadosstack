...
import { Building2, LogOut, UserRoundX } from "lucide-react";
...
type MembershipSuspendedStateProps = {
  workspaceName: string;
  availableWorkspaces: readonly WorkspaceMembershipSummary[];
};

export function MembershipSuspendedState({
  workspaceName,
  availableWorkspaces,
}: Readonly<MembershipSuspendedStateProps>) {
  return (
    <main className="grid min-h-svh place-items-center bg-white px-6 py-12 text-neutral-950">
      <div className="flex w-full max-w-lg flex-col items-center text-center">
        ...
        />
        <p className="mt-2 text-neutral-500 text-sm uppercase tracking-[0.22em]">VadosStack Workspace</p>
        <div className="mt-8 grid size-12 place-items-center rounded-full bg-amber-100 text-amber-700">
          <UserRoundX className="size-6" />
        </div>
        <h1 className="mt-4 text-2xl tracking-tight">Your workspace access is suspended</h1>
        <p className="mt-2 text-balance text-neutral-600">
          Your access to {workspaceName} is temporarily suspended. Its dashboard and business data will remain
          unavailable until a workspace owner or administrator restores your access.
        </p>
        <p className="mt-3 text-balance text-neutral-500 text-sm">
          Your VadosStack account and access to other active workspaces are not affected.
        </p>

        {availableWorkspaces.length ? (
          <div className="mt-8 w-full rounded-xl border border-neutral-200 p-2 text-left">
            <p className="px-2 pt-1 pb-2 font-medium text-neutral-500 text-xs uppercase tracking-wide">
